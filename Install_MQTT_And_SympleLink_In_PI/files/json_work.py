...
def parse_device_topic(json_dictionary):
	...
	return sub_address,dir,message,data_type
...
def get_message_and_data_type_from_sub_address_and_dir(json_dictionary, sub_add, dir):
	...
	while(topic_iterate < total_topics):
		...
		sub_address,direction,message,type = parse_device_topic(topics[topic_iterate])
...
def get_dev_info_from_message(json_dictionary, message):
	...
	while(devs_counter < total_devices):
		...
		while(topic_iterate < total_topics):
			...
			sub_address,direction,msg,type = parse_device_topic(topics[topic_iterate])
			
			if(msg == message):
				return address,sub_address,direction,type
